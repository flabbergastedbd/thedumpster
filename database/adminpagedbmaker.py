...
import sqlite3
conn = sqlite3.connect('adminpage.db')
c = conn.cursor()
tables = [php,asp,cfm,js,cgi,brf]
table_name = ['php','asp','cfm','js','cgi','brf']
"""
counter = 0
same = 0
for i in range(0,len(tables)):
    c.execute('create table '+table_name[i] +'(dork text PRIMARY KEY, score real)')
    for path in tables[i]:
        try:
            temp_var = str('inurl:'+path)
            c.execute("insert into " + table_name[i] +" values ('"+ temp_var +"',0)")
            counter += 1
...
"""
c.execute('select dork from php')
for row in c:
    print(row[0])
conn.commit()
c.close()